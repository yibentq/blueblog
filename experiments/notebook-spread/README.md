# 跨页原型 + 翻页动效（首页"翻书"改版 · 步骤3局部 + 步骤5）

> 承接 `experiments/notebook-cover/README.md`（封面已定稿）。这个目录做的是
> **步骤3的最小验证**（封面+目录页放一起协不协调）和**步骤5**（点击目录条目→
> 全文页的翻页动效），跳过了步骤3剩余细节和步骤4，是站长指定的顺序，不是遗漏。
>
> **2026-09-23 补充**：步骤3剩余的两项（多痕迹页对比、书脊分册标签）已补上原型，
> 见下方"步骤3剩余项补充"。都还是**未经站长确认的判断题版本**，不是定稿。

## 现在这里有什么

- `gen-page-textures.js`：内页"痕迹故事"材质生成脚本，**原样复制自**
  `experiments/book-materials/page-variants.js`（那份没被否决的部分——发霉方向被
  推翻后改成的茶渍/水渍/晒痕/包浆五变体，算法本身站长已确认过，这里只是换了个
  输出目录，没有改算法）。跑法：`node experiments/notebook-spread/gen-page-textures.js`，
  会在本目录生成 `P0-clean.png` ~ `P4-handling-patina.png` 五张贴图。
- `build-spread-prototype.js` → `spread-prototype.html`：**步骤3的最小验证**。
  左页是缩小复用的封面（`notebook-cover` 那套原样搬过来），右页是一页目录，用
  P1（茶渍）内页材质 + 4条占位目录随性排布 + 标题艺术字/正文清晰字体分层。
  只做了一页，**没有**做多痕迹页对比、没有做书脊分册标签——这两项还是
  `docs/BOOK_DESIGN.md` 步骤3 里没做完的部分，跳过了，不是做完了。
- `build-flip-transition-prototype.js` → `flip-transition-prototype.html`：
  **步骤5**，点击目录条目触发"哗啦啦翻过一叠纸"动效，取4张不同痕迹的内页材质
  错开快速翻过，总时长 0.7s（落在规格要求的 0.6–1s 区间），用假延迟
  （550–750ms 随机）模拟异步取全文内容，动画结束和内容到达两者时长故意做得
  接近，用来验证"内容比动画早到/晚到"两种情况下都不会让用户看到空白或跳变。

## 步骤3剩余项补充（2026-09-23）

- `build-patina-comparison-prototype.js` → `patina-comparison-prototype.html`：
  **多痕迹页对比**。同一份排版和文字内容，唯一变量是 P0~P4 贴图，横排对比。
  结论（供站长判断，不是我自己拍板）：五种痕迹在目录页这个尺寸下**差异比预期更细微**——
  茶渍/水渍/晒痕/包浆肉眼要凑近看才分得出，不确定这是"克制得体"还是"做了等于没做"，
  这是审美判断，需要站长看实图定。文字可读性没问题，五种贴图上黑色系正文都够清楚，
  这条可以放心。
- `build-spine-tabs-prototype.js` → `spine-tabs-prototype.html`：
  **书脊分册标签**。在装订缝（`.gutter`）位置加 4 片竖排文字标签，代表 4 个分册。
  设计判断（都未确认）：
  - 当前分册标签**伸出更多、磨损更重**（不是更干净）表示"被翻得最多"，历史册子依次
    收进去、颜色转淡——对应第2节禁忌"磨损不均匀"，但这个"当前=最脏"的直觉可能需要
    站长确认是否认可，不是不言自明的选择。
  - **没有做 hover 效果**，"当前"状态完全靠伸出长度+磨损叠加两个静态信号表达，符合
    禁忌1，但代价是标签本身视觉上偏"扁平矩形卡片"，还是有点像网页侧边栏 tab，
    "物理贴纸感"不够强，这一点我不满意，需要站长看实图判断能不能接受，
    还是要往回形针/丝带书签那类更手作的方向改。
  - 标签文字用 `writing-mode: vertical-rl`，年份数字会横向躺着嵌在竖排文字里，
    36px 宽度下数字基本不可辨认——如果站长要求分册标签本身就能看清年份，这里要改
    （比如只在当前标签显示完整"2026·秋"，历史标签只显示"夏/春/冬"单字，年份放跨页顶部
    `toc-header` 里已经有的位置）。
  - 点击切换分册的交互完全没接（纯静态展示当前态），交互留给步骤4/6。

## 步骤4后端补充（2026-09-23）

真实数据接入的**后端部分**已完成，前端仍是假数据，这两件事分开进度记：

- `src/models/post.js` 新增 `listVolumes` / `listByVolume` / `countByVolume`——分册按
  `published_at` 的年/季度现算（`CEIL(月份/3.0)`），数据库里没有单独的分册字段。
- `src/routes/public.js` 新增三个只读 JSON 接口：
  - `GET /api/notebook/volumes` —— 书脊标签用，返回有文章的年/季度列表（新到旧）
  - `GET /api/notebook/toc?year=&quarter=&page=` —— 目录页用，每页6篇（对应设计规格
    "4-6篇"的上限）
  - `GET /api/notebook/article/:slug` —— 步骤5翻页动效用，只给文章片段（标题/HTML/
    标签），**故意不复用** `/p/:slug` 的渲染逻辑、也故意不在这里累加阅读量（真实阅读量
    应该算在访问 `/p/:slug` 那次，这里只是换内容，前端换上内容后要不要补一次静默计数
    还没做，是个小 TODO）
- 用临时 Postgres + 手工插入跨季度的测试文章验证过：分册分组正确、草稿不会出现在任何
  接口里、`year`/`quarter` 缺失或越界（比如 quarter=9）返回 400、超出范围的分页返回空
  `entries` 而不是报错、**首页 `/` 的服务端渲染列表不受影响**（无 JS/爬虫兜底没被动到）。
- **没做的**：前端完全没接这三个接口。`spread-prototype.html` 里的4条目录、
  `flip-transition-prototype.html` 里的 `setTimeout(fakeLatency)`、`spine-tabs-prototype.html`
  里的4个分册标签，都还是硬编码假数据，需要下一轮把它们换成真实 fetch。

## 步骤4前端对接补充（2026-09-23，第二轮）

只做了**书脊分册标签这一个原型**的对接，没有动 `spread-prototype.html` /
`flip-transition-prototype.html`——一步一步来，这两个还是假数据，是下一轮的事，
不要看到这节标题以为整个步骤4前端都接完了。

- `build-spine-tabs-prototype.js` 生成的页面不再写死 `VOLUMES`/`ENTRIES`，改成
  加载后用 `fetch('/api/notebook/volumes')` 和 `fetch('/api/notebook/toc?...')`
  读真实数据；点击标签会切换分册、重新拉取该分册第1页目录。
- 补上了"点击切换分册"的交互（上一轮标注的"交互留给步骤4/6"里步骤4这部分）。
- 因为要发真实请求，**不能再直接双击本地文件打开**（`file://` 下 fetch 相对路径
  会被浏览器当跨源拦掉）。新增了一条**仅开发环境**的静态路由
  `src/routes/public.js` 里 `if (process.env.NODE_ENV !== 'production')` 包住的
  `/dev/notebook/*`，把这个目录原样透出去。跑 `npm run dev`，浏览器开
  `http://localhost:3000/dev/notebook/spine-tabs-prototype.html` 就是同源。
  已经手工验证过：`NODE_ENV=production` 时这条路由是 404，`/api/notebook/*`
  三个正式接口不受影响；本地起了一份临时 Postgres + 跨季度测试文章，三个接口的
  返回结构、400/404 情况都又核对了一遍，和步骤4后端那节记录的一致，没发现新问题。
- **磨损（`--wear`）和"当前选中"（`.active`）拆成了两件独立的事**（本轮新加的
  工程判断，未与站长确认）：`.active`（标签伸出更多）跟着点击走，磨损深浅是
  每个分册固定的属性、按新旧顺序线性插值，不随点击变化——磨损记录"平时翻不翻"，
  不是"这一刻在不在看"。如果站长觉得"点开哪本哪本就该显脏"更直觉，这里要改。
  代码注释里写了同样的说明，改的时候一起看。
- 还没做：分页控件（现在只取每个分册第1页）、点目录条目的反应（步骤5的翻页动效
  还没并进来）、书脊标签本身"物理贴纸感不够、有点像网页侧边栏 tab"这条上一轮
  就提过的美术顾虑还没处理。
- 没能力做的：视觉截图确认——这一轮的沙箱环境下载不了 headless 浏览器
  （Playwright 的 Chromium 下载源不在允许访问的域名列表里），只验证了接口数据
  和 DOM 结构是对的，没能像 v3/v4 那样截图看实际观感，这条请站长本地打开后确认。

## 步骤4前端对接补充（2026-09-23，第三轮）——第二片：跨页原型

`build-spread-prototype.js` 也接上了真实数据，处理方式和上一轮 spine-tabs 一致：
- 右页目录不再写死 4 条占位，改成 `fetch('/api/notebook/volumes')` 拿最新一册，
  再 `fetch('/api/notebook/toc?...')` 取它的第1页真实文章，渲染进 `.entries`。
- 这个原型本身没有书脊标签、不用切换分册，所以比 spine-tabs 那版简单——只做了
  "拿最新一册第1页"这一次 fetch，分册切换的交互留在 spine-tabs 原型里，没有重复做。
- 同样只能通过 `http://localhost:3000/dev/notebook/spread-prototype.html` 同源访问，
  双击本地文件会因为跨源被拦掉。已用本地临时 Postgres 验证过能正常拿到数据渲染。
- 缩略图仍是占位色块（步骤7）、标题字体仍是未确认的 Caveat 占位，这两点这一轮没动。

至此 `spine-tabs-prototype.html` 和 `spread-prototype.html` 都接了真实数据；
`flip-transition-prototype.html` 里的 `setTimeout(fakeLatency)` 换成真实的
`fetch(slug)` 这一片，见下方"步骤4前端对接补充（第三轮）——第三片"，已在
下一轮单独做完。

## 站长看过跨页原型后的反馈

没有给具体修改意见，直接让继续推进（跳到步骤5），按"协调、可以接受"处理，
但**没有逐项确认**（缩略图占位色块、Caveat 字体、目录条目错位幅度、装订缝处理
都还是未锁定状态，不代表定稿）。

## 步骤4前端对接补充（2026-09-23，第三轮）——第三片：翻页动效收尾

`flip-transition-prototype.html` 也接上了真实数据，是这一轮"跳过流程"里最后
一块假数据：

- 目录条目不再写死3条假标题，改成和 `spread-prototype.html` 同一个来源——先
  `fetch('/api/notebook/volumes')` 拿最新一册，再 `fetch('/api/notebook/toc?...')`
  取第1页真实文章，每条目录记住自己的 `slug`。
- 点击后原来的假延迟 Promise（`setTimeout(fakeLatency)`）换成真正的
  `fetch('/api/notebook/article/:slug')`，动效本身（错开时间、旋转轨迹、总时长
  0.7s）完全没动——只换了触发信号源，符合当时立的计划。
- 全文页内容是真实标题 + `content_html`；服务端 `/api/notebook/article/:slug`
  给的就是保存时净化过的字段，和 `/p/:slug` 走同一份 `sanitize-html` 管线，这里
  直接 `innerHTML` 不是新开的风险面（已核对 `src/routes/public.js` 对应路由的
  实现，不是凭记忆假设）。
- 加了取数失败时的降级提示（顶部提示条变红，说明依赖 `/api/notebook/*`、
  必须走 `npm run dev` + `/dev/notebook/...` 同源访问，不能双击本地文件打开）。
- 阅读量静默计数那个小 TODO 仍然没做（原作者标注过"这里先不做"），这一轮没有
  顺带加，不属于"重新接线"的范围。

**验证方式**：这一轮沙箱里装了一次性本地 Postgres（能装，跟之前"下不了
Playwright Chromium"是两回事，之前几轮记录的坑不代表这次也不能装库），跑了
`npm run migrate && npm run seed`，起了真实的 `src/app.js`，用 `curl` 真实打了
四个请求验证：`/api/notebook/volumes`、`/api/notebook/toc?...`、
`/api/notebook/article/<真实slug>`（确认返回里含真实标题和正文 HTML）、
`/api/notebook/article/<不存在的slug>`（确认返回 404）、以及
`/dev/notebook/flip-transition-prototype.html`（确认同源静态路由能访问到）。
**沙箱依旧下不了 Playwright 的 Chromium**（下载源不在允许域名列表），所以
**没能像 v3/v4 那样截图看点击后的真实动效观感**——只验证了接口数据和后端
渲染逻辑是对的，翻页动效本身的手感、内容淡入的实际效果，请站长本地打开
`http://localhost:3000/dev/notebook/flip-transition-prototype.html` 点一下确认。

至此 `spine-tabs-prototype.html`、`spread-prototype.html`、
`flip-transition-prototype.html` 三个原型全部接上了真实数据，步骤4前端对接
这一片可以算完成；**动效手感本身（时长、翻页轨迹、内容淡入方式）站长还没给
过具体反馈**，不代表定稿。

## 步骤5补充（2026-09-23，第四轮）——返回目录 + 阅读量静默计数

上一轮记录的两个"还没做"这一轮补上了：

- **返回目录动效**：全文页顶部加了"← 返回目录"按钮。点击后复用同一批
  `.flying-page` 元素反向播放一次（轨迹、错开顺序、总时长都和"进入"对称，
  不是另起一套动效），播完把全文页收起、目录重新显示、条目重新可点。播放期间
  按钮会 disable，防止连点导致两套动效叠在一起。
- **阅读量静默计数**：新增 `POST /api/notebook/article/:slug/view`
  （`src/routes/public.js`），复用已有的 `postModel.incrementViewCount`，
  不新增字段、不做去重——口径和 `/p/:slug` 现有逻辑一致（每次访问都计一次）。
  前端在全文内容成功渲染后调用一次，用普通 `fetch(..., {method:'POST'})`
  而不是 `sendBeacon`：这里不是页面卸载场景（用户可能点返回又点别的文章），
  `sendBeacon` 语义对不上。失败了静默忽略，不影响动效和内容展示。

**验证方式**：本地重新起了一次性 Postgres + 真实 `src/app.js`，用 curl 验证了
新端点——真实 slug 返回 204 且 `posts.view_count` 真的 +1，不存在的 slug 返回
404；也确认了原型页面本身还能正常访问（200），生成的 HTML 里新逻辑
（`backToToc`/`backBtn`/`/view` 调用）都在。**同样受限于沙箱下不了 Playwright
Chromium**，没能实际点一下"返回"按钮看反向动效的真实观感是否顺畅、有没有
视觉跳变，这条和"进入"动效一样需要站长本地确认。

至此步骤5列出的两个补充项都已实现，但**都还没有站长的手感反馈**，不算定稿；
如果返回动效的方向感、时长和进入不对称观感不对，需要调的话在
`runFlying(direction, onDone)` 这一个函数里改就行，进入/返回共用同一套参数。

## 预览方式改动（2026-09-23，第五轮）——`/dev/notebook/*` 从"仅开发环境"改成"登录可见"

站长部署上线后按提示 `git pull` 完发现首页没变化——这是预期内的（这几轮动效
只在 `/dev/notebook/*` 原型页里，没碰首页），但站长要在**生产环境**看这批原型时，
发现 `/dev/notebook/*` 原来是 `NODE_ENV !== 'production'` 才挂载的，生产环境是
404，每次预览都得临时把 `.env` 里的 `NODE_ENV` 改成 `development` 再改回去，
麻烦且有风险（切换期间生产专用的限流/安全 cookie 也会跟着松开）。

**改法**（`src/routes/public.js`）：这条路由不再判断 `NODE_ENV`，改成套
`requireAuth`（和 `/admin/*` 其他路由一样的登录校验中间件）——登录管理员账号
就能在生产环境直接访问 `/dev/notebook/flip-transition-prototype.html` 等页面，
未登录会跳转到 `/admin/login`。三个只读接口 `/api/notebook/*` 不受影响，
仍然公开（本来就是只读展示数据，和 `/feed.xml` 一个安全等级）。

**已知的小别扭**：登录后会跳回 `/admin` 首页，不会自动回到刚才想看的那个原型
页面（`requireAuth` 现有实现没做 `returnTo`），得手动再输一次
`/dev/notebook/...` 的地址——这是复用现成中间件的代价，没有为这一个场景单独
加 `returnTo` 逻辑，如果站长觉得麻烦可以再提。

**验证方式**：这一轮在沙箱里装了一次性本地 Postgres，跑了真实 `src/app.js`，
分别用 `NODE_ENV=production` 和 `NODE_ENV=development` 两种模式各验证了一遍：
- production 模式：不带登录 session 直接访问
  `/dev/notebook/flip-transition-prototype.html` 返回 302 跳 `/admin/login`，
  `/api/notebook/volumes` 仍返回 200。**没能测通 production 模式下带登录态的
  完整流程**——生产环境 session/CSRF cookie 都带 `Secure` 属性（CSRF cookie还是
  `__Host-` 前缀），必须走 HTTPS，沙箱里只有 HTTP，登录后 `Set-Cookie` 虽然
  下发了，但 `Secure` cookie 不会在后续 HTTP 请求里带回去，所以这条路径的
  "登录后能不能看到"这一步只在 development 模式下测通，不是 production 模式
  没做校验、而是沙箱的 HTTP 限制测不出来。
- development 模式（cookie 不带 `Secure`）：走了一遍真实的 `curl` 登录
  （拿 `_csrf` 隐藏字段 → 连同 `__Host-csrf` cookie 一起 POST `/admin/login`
  → 拿到 `blog_blue_sid` session cookie），带着登录 cookie 访问
  `flip-transition-prototype.html` 和 `spread-prototype.html` 都返回 200，
  不带登录 cookie 仍是 302——确认了"未登录挡住、登录放行、只读接口不受影响"
  这三点都对；`requireAuth` 是复用 `/admin/*` 现成、已经在生产跑着的中间件，
  不是新写的校验逻辑，风险可控。**站长部署后请实际登录一次 `/admin`，
  再访问 `/dev/notebook/flip-transition-prototype.html` 确认能在生产环境看到。**

## 步骤5动效目前的坑 / 下一步

- 还没问过站长这版动效的手感——时长、翻页轨迹、内容淡入方式、返回动效的
  方向感都可能要调。
- 阅读量计数目前没有做"同一用户重复点开算不算多次"的判断，和 `/p/:slug`
  一致，如果站长觉得应该去重，需要单独排期改（两处口径要一起改，不要只改
  一处）。

## 目录页之间的翻页（2026-09-23，第六轮）——`spread-prototype.html` 加了分页

**编号更正**：这份 README 之前几轮把"目录页之间的慢速物理翻页"写成"步骤6"，
是误把 `docs/BOOK_DESIGN.md` 第6节（讲两级翻页动效的那一节）的节号当成了第9节
执行计划里的步骤编号——查了 `docs/BOOK_DESIGN.md`，**执行计划里真正的步骤6是
"标签/搜索便签系统"**（对应文档第5节），是完全独立的另一块，还没有做，也不在
这一轮的范围内。下面记的是第6节里提到、`spread-prototype.html` 之前一直没接的
"上一页/下一页"分页交互，不要再按"步骤6"理解。

这一轮做的：
- 右页目录底部加了"‹ 上一页 / 下一页 ›"按钮，接的是 `/api/notebook/toc` 本来就
  支持的 `page` 参数（后端接口不用改，前端之前只用了 `page=1`）。
- 动效走"慢速物理翻书"路线，和步骤5的快速哗啦啦刻意区分：`.toc-page` 以左侧
  装订缝为轴心做 CSS 3D `rotateY`，转到 90°（侧面看不见内容）那一瞬间换上新
  数据，再转回 0°，总时长 900ms（步骤5是 700ms，明显更快）。
- 翻页期间按钮 disable 防连点；到第一页/最后一页对应按钮 disable（不隐藏，
  让用户看得出"到头了"）；请求失败会把页面转回原样并显示错误提示，不会卡在
  侧面转不回来。
- 沿用同一份 P1 内页材质，没有另开新的贴图。

**验证方式**：本地起了真实 `src/app.js` + 一次性 Postgres，手工插入11篇测试
文章让同一分册产生2页数据，用 curl 确认了 `/api/notebook/toc?...&page=1` 和
`page=2` 返回的 `total`/`totalPages`/`entries` 都对；登录后访问
`/dev/notebook/spread-prototype.html` 返回 200，生成的 HTML 里 `prevBtn`/
`nextBtn`/`flipToPage` 相关标记都在。**同样卡在沙箱下不了 Playwright
Chromium**，没能实际点"下一页"看翻页动效的真实观感（转到侧面换内容那一下
顺不顺、有没有闪烁），这条请站长本地确认。测试插入的11篇文章是本地临时
数据，没有进这次提交。

## 仍然跳过、没做的部分（对照 `docs/BOOK_DESIGN.md` 步骤清单）

- 步骤3：多痕迹页对比、书脊分册标签**已出原型**（见上），但**都未经站长确认**，
  不算完成；标签的点击交互仍未接
- 步骤4：目录翻页交互接真实文章数据——**三片（书脊分册标签、跨页原型、翻页动效）
  均已接**（见上面三节"步骤4前端对接补充"），前端对接这一片可以算完成；
  `spread-prototype.html` 的目录页分页交互也已补上（见上一节）
- 步骤6：标签/搜索便签系统（对应第5节），还没做——**不是**之前几轮误写的
  "目录页之间的慢速物理翻页"，那部分已在上一节做完
- 步骤7：真实图片装饰系统（胶带/拍立得/大头针），当前用占位色块代替
- 步骤8/9：移动端隔离确认、SEO/无 JS 兜底验证、禁忌清单逐条走查

## 给接手的人

按项目协作约定：一步一步做，每步先分析再操作。这个目录里的顺序是站长临时
指定的跳跃顺序（先5后3/4），**接手时不要假设步骤是按数字顺序完成的**，
以这份 README 和上面"仍然跳过"清单为准，不要看 `docs/BOOK_DESIGN.md` 的
步骤清单打勾状态就以为3、4已经做完。
