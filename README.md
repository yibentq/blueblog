...
- **图片水印 / 防伪**：上传的图片会自动打上签名水印，水印里叠了四层防伪（扭索纹、微缩文字、刻度暗码、陷阱字符），序列号由 `HMAC-SHA256(WATERMARK_SECRET, 文件名)` 推出，不入库——只要密钥没变，随时能拿原图重算比对。查证用 `npm run wm:verify -- <文件名>`。细节和设计取舍见 `docs/HANDOFF.md`。
...
- **品牌/防伪角度**：`blog.blue` 的词标是手工拟合的单线条连笔签名（`src/utils/brand.js`），不是任何一款现成字体——专属性、不依赖服务器装了什么字体、以及"描摹必走形"这三件事是一举拿下的。页眉词标、favicon、图片水印同源于这一份路径数据。
...
- 水印宽度按图片宽度的 26% 自适应，150–320px 之间夹住，避免小图挤、大图水印太小看不清
